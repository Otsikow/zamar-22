...
-- TYPES
-- ===============================
CREATE TYPE referral_status AS ENUM ('clicked','signed_up','purchased');
CREATE TYPE earning_status AS ENUM ('pending','locked','paid','void');
CREATE TYPE payout_status AS ENUM ('queued','processing','paid','failed');

-- Commission rates table (so you can change later without code)
...
-- Add L2 referrer tracking to referrals table
ALTER TABLE referrals 
ADD COLUMN IF NOT EXISTS l2_referrer_id uuid REFERENCES profiles(id) ON DELETE SET NULL;
...
CREATE INDEX IF NOT EXISTS idx_referrals_l2 ON referrals(l2_referrer_id);
CREATE INDEX IF NOT EXISTS idx_ref_earnings_level ON referral_earnings(referrer_id, level, status);
...
BEGIN
  ...
  -- Mark as purchased
  UPDATE referrals SET status='purchased', updated_at=now() WHERE id=v_ref.id;
  ...
  v_l1_cents := floor(p_gross_amount_cents * v_rate_l1);
  ...
  v_l2_cents := CASE 
    WHEN v_l2_referrer_id IS NOT NULL THEN floor(p_gross_amount_cents * v_rate_l2) 
    ELSE 0 
  END;

  -- Create L1 earning
  INSERT INTO referral_earnings(
    referrer_id, referred_user_id, referral_id, level, currency,
    amount, status, locked_until
  ) VALUES (
    v_ref.referrer_id, v_ref.referred_user_id, v_ref.id, 'L1', p_currency,
    v_l1_cents, 'locked', now() + make_interval(days => p_locked_days)
  );
  ...
  IF v_l2_referrer_id IS NOT NULL AND v_l2_cents > 0 THEN
    INSERT INTO referral_earnings(
      ...
      v_l2_referrer_id, v_ref.referred_user_id, v_ref.id, 'L2', p_currency,
      v_l2_cents, 'locked', now() + make_interval(days => p_locked_days)
    );
  END IF;
END $$;
