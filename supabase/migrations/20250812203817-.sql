-- Add missing columns to custom_song_requests (idempotent)
...
-- Duration constraint if not exists
DO $$ BEGIN
  IF EXISTS (
    ...
    IF NOT EXISTS (
      SELECT 1 FROM pg_constraint WHERE conname = 'custom_song_requests_duration_seconds_check'
    ) THEN
      ALTER TABLE public.custom_song_requests
...
-- updated_at trigger using existing function (idempotent)
DO $$ BEGIN
  IF NOT EXISTS (
