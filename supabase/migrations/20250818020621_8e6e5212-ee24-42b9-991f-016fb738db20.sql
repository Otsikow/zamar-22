...
create or replace function public.generate_referral_code_v2(uid uuid)
returns text
language plpgsql
as $$
declare
...
-- Update existing profiles to have referral codes if missing
update public.profiles set referral_code = public.generate_referral_code_v2(id) where referral_code is null;
...
begin
  if new.referral_code is null then
    new.referral_code := public.generate_referral_code_v2(new.id);
  end if;
  new.updated_at := now();
  return new;
end;
...
create index if not exists idx_orders_v2_user on public.orders_v2(user_id);
create index if not exists idx_orders_v2_status on public.orders_v2(status);

create or replace function public.orders_v2_set_updated_at()
...
drop trigger if exists trg_orders_v2_set_updated_at on public.orders_v2;
create trigger trg_orders_v2_set_updated_at
before update on public.orders_v2
for each row execute procedure public.orders_v2_set_updated_at();

-- === COMMISSIONS (records Gen-1 + Gen-2 earnings) ===
create table if not exists public.referral_commissions (
  id uuid primary key default gen_random_uuid(),
  source_order_id uuid not null references public.orders_v2(id) on delete cascade,
...
drop trigger if exists trg_orders_v2_create_commissions_insert on public.orders_v2;
create trigger trg_orders_v2_create_commissions_insert
after insert on public.orders_v2
for each row execute procedure public.create_commissions_for_order();

drop trigger if exists trg_orders_v2_create_commissions_update on public.orders_v2;
create trigger trg_orders_v2_create_commissions_update
after update on public.orders_v2
for each row execute procedure public.create_commissions_for_order();
...
for select using (earner_user_id = auth.uid() or is_admin());
...
drop policy if exists p_orders_v2_read_own on public.orders_v2;
create policy p_orders_v2_read_own on public.orders_v2
for select using (user_id = auth.uid() or is_admin());

drop policy if exists p_orders_v2_insert_own on public.orders_v2;
create policy p_orders_v2_insert_own on public.orders_v2
for insert with check (user_id = auth.uid() or is_admin());

drop policy if exists p_orders_v2_update_own on public.orders_v2;
create policy p_orders_v2_update_own on public.orders_v2
