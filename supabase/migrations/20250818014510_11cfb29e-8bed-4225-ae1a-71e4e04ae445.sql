...
CREATE OR REPLACE VIEW public.v_top_referrers_30d_named AS
SELECT
  e.user_id as referrer_id,
  COALESCE(
    p.display_name,
...
-- Overview cards view
CREATE OR REPLACE VIEW public.v_admin_referral_overview AS
SELECT
...
CREATE OR REPLACE VIEW public.v_admin_payout_queue AS
SELECT
  user_id as payee_id,
  COALESCE(
    p.display_name,
...
CREATE POLICY "Admins can view referral earnings detailed" ON referral_earnings
